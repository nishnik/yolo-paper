...
We define a function $Y:\mathbf{X} \rightarrow \mathbb{R}$ such that $Y(x)$ for every $x \in \mathbf{X}$ denotes the time spent at the location \textit{tag} $x$. For example, let $x = \text{gym}$. Say $Y(x) = 0.5$. This implies a person has spent $30$ minutes at a \textit{gym} in the entire day.
...
We define the following fuzzy sets in the following categories. These sets define the type of lifestyle a person is living in each category. Here \textit{leisure} also includes rest.
\begin{align*}
...
\]
Similarly we define the other scores. The fuzzy set of linguistic terms "low\_score", "ideal\_score" and "high\_score" would define the fuzzy scores in each category. The membership function of these sets in all categories would be calculated similar to the fuzzy time membership functions by conducting a survey. For instance, a survey conducted on a sample of fit students is shown in TABLE II.
\begin{table}
\small
\captionof{table}{Sample survey to determine the membership function for the linguistic term "ideal\_score" under \textit{health} category}
\begin{center}
\def\arraystretch{1.7}
\begin{tabular}{| l | l | l |}
\hline
...
\end{center}
\end{table}
Hence $\inf = 11.25,\ Q_1=29.75,\ Q_3=42,\ \sup = 50$.  Accordingly the membership function for the linguistic term "ideal\_score" under the health category is shown in Figure 5. Similarly we can plot plot the membership functions for the entire fuzzy set across all categories.
\begin{figure}[h!]
\centering
\captionsetup{justification=centering}
...
\section{Experiment}
A survey conducted in IIT Kharagpur was used to determine all the membership functions for all linguistic terms across all the categories. Some of the membership functions are shown in this paper. We also installed the mobile application on students' smart phones and analysed the results. We picked a random student and analysed his data for a day. TABLE III shows the \textit{tags} he visited throughout the day and their corresponding time and weights. The score for each \textit{tag} is enumerated. TABLE IV denotes the net time and score across all the categories. We considered the recommendations in the set $R$ where $R = \{R_1, R_2, R_3, R_4\}$.
\begin{align*}
